...
| Phrase | Action |
|---|---|
| **catch up** | Read `SESSION_SUMMARY.md` (auto-generated by the llm-mem daemon) or call `mem_get_briefing`. Summarize the current project state and ask what to work on. |
...
llm-mem captures every Claude Code / OpenCode event and extracts entities (decisions, todos, facts, features, bugfixes, discoveries, failures) into `.llm-mem/memory.db`. The daemon rewrites `SESSION_SUMMARY.md` on every extraction pass, so the next session's briefing is always fresh.

**No flat memory files.** Don't write to `.llm-mem/SESSION.md`, `TODO.md`, or `DECISIONS.md` — those were removed. The DB is the source of truth; the briefing reads from it.
