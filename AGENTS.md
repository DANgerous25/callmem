...
1. **At the start of every non-trivial task**, call `mem_get_briefing` once. The briefing surfaces recent decisions, open TODOs, unresolved failures, and recent session hooks. It costs ~2k tokens once and routinely saves you re-reading 5–20 files.
2. **Before reading a file you suspect was touched before**, call `mem_file_context path=<file>`. If the timeline covers what you need, skip the raw read.
3. **Before asking the user a clarifying question**, call `mem_search query=<topic>`. There's a good chance an earlier session already decided it.
...
Examples:
- *"Per `#R7BAFG3Y`, this was the uv shim issue — the fix is to reinstall the global tool."*
- *"There's an unresolved failure on this (`#FPCHSXRM`), so I'll start by reproducing it before changing anything."*
- *"`#NBPRHE0Y` already tracks the version-string sync TODO — I'll close that one out as part of this commit."*

Why this matters:
- The `callmem usage` analytics counts citation matches against real entity IDs in the DB. **A citation is the only countable proof that memory informed a response** — without citations, memory looks unused even when it's read.
- A reader (you in two weeks, the user, the next agent) can pull up any cited entity in a second to see the full context.
...
### Starting a task
1. Run `callmem briefing` or call `mem_get_briefing` via MCP
