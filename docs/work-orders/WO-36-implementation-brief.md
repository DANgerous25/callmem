...
```
https://github.com/DANgerous25/callmem
```

Python project. `src/callmem/` layout. Tests in `tests/`. Run tests with `pytest`. Lint with `ruff check .`.

## What to read first

1. `AGENTS.md` — project norms and conventions
2. `docs/work-orders/WO-36-claude-code-mcp.md` — the full spec
3. `src/callmem/cli.py` — the main file you'll modify (setup wizard + init command)
4. `src/callmem/core/config.py` — config template generation
5. `tests/` — existing test patterns to follow

## What this work order does

callmem is a persistent memory system for coding agents. It currently only integrates with OpenCode (via `opencode.json` MCP config). This WO adds Claude Code support (via `.mcp.json`), so both tools can share the same memory database.

## Key functions to understand

In `cli.py`:

- `_detect_mcp_command(project)` — figures out the right Python path to run the MCP server. Returns a list like `["python3", "-m", "callmem.mcp.server", "--project", "."]`. **Reuse this** for Claude Code.
...
- Check if `mcpServers.callmem` already exists with correct command — if so, no-op
- Split `_detect_mcp_command()` result into command (first element) and args (rest)
- Write/update the `callmem` entry under `mcpServers`
