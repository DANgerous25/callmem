...
per-file byte offset, and streams new events into llm-mem. Mirrors
...
Session lifecycle:
    - On the first record in a previously unseen file, open a new
      llm-mem session tagged with the file's source_id.
...
Offsets are persisted to ``.llm-mem/claude_code_offsets.json`` so a
...
OFFSETS_FILE = Path(".llm-mem") / "claude_code_offsets.json"
