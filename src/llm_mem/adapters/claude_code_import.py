...
historical Claude Code sessions into llm-mem. Counterpart to
...
PROGRESS_FILE = Path(".llm-mem") / "claude_code_import_progress.json"
LOCK_FILE = Path(".llm-mem") / "claude_code_import.lock"
...
def project_slug(project_path: Path) -> str:
    ...
    ``/home/dan/llm-mem`` becomes ``-home-dan-llm-mem``.
    """
...
def _find_imported_session_id(
    ...
    """Return the llm-mem session id previously imported from this CC file."""
...
def import_session(
    ...
    """Replay a single CC transcript into llm-mem.
