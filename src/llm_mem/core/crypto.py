...
(AES-128-CBC + HMAC). Two key modes:
  - auto: random key stored in .llm-mem/vault.key (default)
  - passphrase: key derived from LLM_MEM_VAULT_PASSPHRASE env var via scrypt
...
class VaultKeyManager:
    ...
    def __init__(self, llm_mem_dir: Path, mode: str = "auto") -> None:
        self.llm_mem_dir = llm_mem_dir
        self.mode = mode
    ...
    def _from_auto_key(self) -> Fernet:
        ...
        key_path = self.llm_mem_dir / "vault.key"
        if key_path.exists():
    ...
    def _from_passphrase(self) -> Fernet:
        ...
        passphrase = os.environ.get("LLM_MEM_VAULT_PASSPHRASE")
        if not passphrase:
            raise ValueError(
                "Vault mode is 'passphrase' but LLM_MEM_VAULT_PASSPHRASE "
                "environment variable is not set."
            )

        salt_path = self.llm_mem_dir / "vault.salt"
        if salt_path.exists():
